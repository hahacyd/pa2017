...
static uint32_t is_inited = 0;
static void init_filestate();
...
size_t fs_read(int fd, void *buf, size_t len) {
	...
	/*if(files[fd + 3].offset >= file_table[fd].size){
		return 0;
	}*/
	...
	assert(files[fd].offset + len <= file_table[fd - 3].size);

	ide_read(buf,file_table[fd - 3].disk_offset + files[fd].offset,len);

	files[fd].offset += len;

	//Log("buf = %x offset = %d fd = %d\n",len,disk_offset,fd);
	return len;
...
size_t fs_write(int fd, void *buf, size_t len) {
	...
	/*if(files[fd + 3].offset >= file_table[fd].size){
		...
		ide_write(buf,file_table[fd - 3].disk_offset + files[fd].offset,len);
...
off_t fs_lseek(int fd, off_t offset, int whence) {
	...
	assert(offset <= file_table[fd - 3].size);
	uint32_t off_res = files[fd].offset;
	switch(whence){
		case SEEK_SET:off_res = offset;break;
		case SEEK_CUR:off_res += offset;break;
		case SEEK_END:off_res = offset + file_table[fd - 3].size;break;
		default:assert(0);
	}
	files[fd].offset = off_res;
	return off_res;
	//return -1;
}
