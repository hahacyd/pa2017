...
void alu_test_sub() {
	...
	uint32_t res, a, b, res_asm, res_eflags;
	TEST_EFLAGS test_eflags;
	int input[] = {0x80000000,0x7fffffff,0x10000000,-3, -2, -1, 0, 1, 2};
	int n = sizeof(input) / sizeof(int);
	int i, j;
	for(i = 0 ; i < n ; i++) {
		for(j = 0 ; j < n ; j++) {
	...
	for(i = 0 ; i < n ; i++) {
		for(j = 0 ; j < n ; j++) {
			...
			if(test_eflags.OF != cpu.eflags.OF){
				printf("hello? this is the first step error,\n");	
				printf("oracle eflags CF = %d, PF = %d, ZF = %d, SF = %d, OF = %d\n", test_eflags.CF, test_eflags.PF, test_eflags.ZF, test_eflags.SF, test_eflags.OF);
...
//					printf("res = %x\nres_asm = %x\n",res,res_asm);
	...
	for(i = 0 ; i < n ; i++) {
		for(j = 0 ; j < n ; j++) {
			...
			//printf("nemu   eflags CF = %d, PF = %d, ZF = %d, SF = %d, OF = %d\n", cpu.eflags.CF, cpu.eflags.PF, cpu.eflags.ZF, cpu.eflags.SF, cpu.eflags.OF);
			//printf("a = %d, b= %d, res = %d, res_asm = %d\n", a, b, res, res_asm);
				if(test_eflags.PF != cpu.eflags.PF  || test_eflags.CF != cpu.eflags.CF|| test_eflags.ZF != cpu.eflags.ZF || test_eflags.CF != cpu.eflags.CF || res != res_asm || test_eflags.SF != cpu.eflags.SF){
				printf("hello? this is the second step error,\n");	
				printf("oracle eflags CF = %d, PF = %d, ZF = %d, SF = %d, OF = %d\n", test_eflags.CF, test_eflags.PF, test_eflags.ZF, test_eflags.SF, test_eflags.OF);
	...
	for(n = 0 ; n < 3 ; n++) {
		for(i = 0 ; i < na ; i++) {
			for(j = 0 ; j < nb ; j++) {
				...
				//printf("nemu   eflags CF = %d, PF = %d, ZF = %d, SF = %d, OF = %d\n", cpu.eflags.CF, cpu.eflags.PF, cpu.eflags.ZF, cpu.eflags.SF, cpu.eflags.OF);
				//printf("a = %d, b= %d, res = %d, res_asm = %d\n", a, b, res, res_asm);
				if(test_eflags.PF != cpu.eflags.PF || test_eflags.ZF != cpu.eflags.ZF || test_eflags.CF != cpu.eflags.CF || res != res_asm || test_eflags.SF != cpu.eflags.SF){
					
				printf("oracle eflags CF = %d, PF = %d, ZF = %d, SF = %d, OF = %d\n", test_eflags.CF, test_eflags.PF, test_eflags.ZF, test_eflags.SF, test_eflags.OF);

				printf("nemu   eflags CF = %d, PF = %d, ZF = %d, SF = %d, OF = %d\n", cpu.eflags.CF, cpu.eflags.PF, cpu.eflags.ZF, cpu.eflags.SF, cpu.eflags.OF);
					printf("a = %x, b= %x, res = %x, res_asm = %x,data_size = %d\n", a, b, res, res_asm,data_sizes[n]);
//					printf("res = %x\nres_asm = %x\n",res,res_asm);
				}
				assert(res == res_asm);
				assert(cpu.eflags.CF == test_eflags.CF);
...
void alu_test_mul() {
	...
	for(i = 0 ; i < n ; i++) {
		for(j = 0 ; j < n ; j++) {
			...
			temp_a = res_asm_a;
			temp_d = res_asm_d;
				if( test_eflags.OF != cpu.eflags.OF || test_eflags.CF != cpu.eflags.CF){
					printf("hello?here is first error]n");					
					printf("oracle eflags CF = %d, PF = %d, ZF = %d, SF = %d, OF = %d\n", test_eflags.CF, test_eflags.PF, test_eflags.ZF, test_eflags.SF, test_eflags.OF);
...
//					printf("res = %x\nres_asm = %x\n",res,res_asm);
			...
			assert(res == (temp_a | (temp_d << 32)));               //temp_d show higher 32 bit, temp_a show lower 32 bit.
			assert(cpu.eflags.CF == test_eflags.CF);
			assert(cpu.eflags.OF == test_eflags.OF);
		}
	}
...
//					printf("res = %x\nres_asm = %x\n",res,res_asm);
	...
	for(i = 0 ; i < n ; i++) {
		...
		if( test_eflags.OF != cpu.eflags.OF || test_eflags.CF != cpu.eflags.CF){
					...
					printf("hello?here is second error\n");					
					printf("oracle eflags CF = %d, PF = %d, ZF = %d, SF = %d, OF = %d\n", test_eflags.CF, test_eflags.PF, test_eflags.ZF, test_eflags.SF, test_eflags.OF);

					printf("nemu   eflags CF = %d, PF = %d, ZF = %d, SF = %d, OF = %d\n", cpu.eflags.CF, cpu.eflags.PF, cpu.eflags.ZF, cpu.eflags.SF, cpu.eflags.OF);
//					printf("res = %x\nres_asm = %x\n",res,res_asm);

			printf("a = %x, b= %x, res = %x, res_asm = %x\n", a, b, (uint32_t)res, (res_asm_a | (res_asm_d << 16)));
}
