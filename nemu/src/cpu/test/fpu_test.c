	...
	for(i = 0 ; i < sizeof(input)/sizeof(float) ; i++) {
		for(j = 0 ; j < sizeof(input)/sizeof(float) ; j++) {
			...
			if(res_fpu.val != res.val){
				...
				//printf("a = %x\nb = %x\n",((FLOAT)a.fval)).val,(uint32_t)b.fval);
				printf("res_asm = %x\nres_fpu = %x\n",res.val,res_fpu.val);

			printf("float add a = %f, b = %f\nua = %x, ub = %x, \nres     = %x \nres_fpu = %x\nres = %f, res_fpu = %f\n", a.fval, b.fval, a.val, b.val, res.val, res_fpu.val, res.fval, res_fpu.fval);
			printf("below is report:\n");
			printf("fpu: sign = %x  exp = %x sig = %x\n",res_fpu.sign,res_fpu.exponent,res_fpu.fraction);
			printf("asm: sign = %x  exp = %x sig = %x\n",res.sign,res.exponent,res.fraction);
			printf("a  : sign = %x  exp = %x sig = %x\n",a.sign,a.exponent,a.fraction);
			printf("b  : sign = %x  exp = %x sig = %x\n",b.sign,b.exponent,b.fraction);
			}	
...
void fpu_test_mul() {
	...
	for(i = 0 ; i < sizeof(input)/sizeof(float) ; i++) {
		for(j = 0 ; j < sizeof(input)/sizeof(float) ; j++) {
			...
			//printf("float mul a = %f, b = %f, \nua = %x, ub = %x, \nres = %x, res_fpu = %x, res = %f, res_fpu = %f\n", a.fval, b.fval, a.val, b.val, res.val, res_fpu.val, res.fval, res_fpu.fval);
