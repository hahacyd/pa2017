...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs) {
	...
	//printf("i\n");
	if((sig_grs >> (23 + 3)) > 1 || exp < 0) {    //need right gui
		// normalize toward right
		uint32_t stick = 0;
		while((((sig_grs >> (23 + 3)) > 1) && exp < 0xff) // condition 1
			|| // or
			(sig_grs > 0x04 && exp < 0) // condition 2 is for * and / ?
			) {
			stick |= sig_grs & 0x1;
			sig_grs >>= 1;
			sig_grs |= stick;
...
//			assert(0);
	...
	sig_grs &= 0x3ffffff;  // remove the hide "1" this is important
//	printf("sig_grs = %x\n",(uint32_t)sig_grs);
	if(!overflow) {
		/* TODO: round up and remove the GRS bits */
		uint32_t mm = sig_grs & 0x7;
		if((4 == mm) &&((sig_grs >> 3) & 1)){
				sig_grs += 8;
		}
		else if(4 < mm){
...
uint32_t internal_float_add(uint32_t b, uint32_t a) {
	...
	uint32_t sticky = 0;          //shift is the times of " a "right gui
	while(shift > 0) {
...
void fpu_div(uint32_t val) {
	...
	float c = *a / *b;
	// printf("f %f / %f = %f\n", *a, *b, c);
	uint32_t *d = (uint32_t *)&c; fpu.regStack[fpu.status.top].val = *d;
	*/
