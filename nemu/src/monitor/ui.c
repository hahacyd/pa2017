...
cmd_handler(cmd_si) {
	...
	char * steps = strtok(NULL, " ");  //strtik() is to desperate strings!
	if(steps == NULL) {
		exec(1);
	} else {
		int n = 1;
...
cmd_handler(cmd_p) {
	...
	//args = strtok(NULL, " ");  //strtik() is to desperate strings!
	bool success;
...
cmd_handler(cmd_b) {
	...
	args += strspn(args, " ");          //to skip " ";
	if(*args == '*') {
		args ++;
		addr = expr(args, &success);
		if(!success) {
			printf("invalid expression or expr function is not comleted: '%s'\n", args);
			return 0;
		}
