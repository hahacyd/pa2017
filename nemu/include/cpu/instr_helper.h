...
#define decode_data_size_near opr_src.data_size = opr_dest.data_size = data_size;

// determine the type and address of operand(s)
// possible types: r, rm, i, m, a, c, o
// here c stands for CL, CX, ECX
#define decode_operand_portdx2a \
...
#define decode_operand_a2portdx \
	opr_dest.type = OPR_REG; \
	opr_dest.data_size = 16; \
	opr_dest.addr = REG_DX;  \
	opr_src.type = OPR_REG; \
	opr_src.addr = REG_EAX;
...
#define decode_operand_cr2r \
	len += modrm_rm(eip + 1,&opr_src);

#define decode_operand_r2sr \
	len += modrm_r_rm(eip + 1, &opr_src, &opr_dest); \
	opr_dest.type = OPR_SREG;

#define decode_operand_sr2r \
	len += modrm_r_rm(eip + 1, &opr_dest, &opr_src);\
...
// conditions
// possible condition: e, a, ae, b, be, o, p, s , ne, na,nb, no, np, ns, g, ge, l, le, ecxz
...
// this is define by me, to solve eAX_IV ,which appear 8 times in same style!
#define self_design_make_instr_eAX_Iv(instr_name)\
	make_instr_func(concat(instr_name,_eax)){\
		int len = 1;\
		decode_data_size_l;\
		decode_operand_i;\
		opr_dest.type = OPR_REG;\
		...
		instr_execute_2op();\
		print_asm_2(#instr_name,"",len,&opr_src,&opr_dest);\
		return len;\
	}
// this is define by me, to solve AL_Ib ,which appear 8 times in same style!  
#define self_design_make_instr_AL_Ib(instr_name)\
	make_instr_func(concat(instr_name,_AL_Ib)){\
		int len = 1;\
		decode_data_size_b;\
		decode_operand_i;\
		opr_dest.type = OPR_REG;\
